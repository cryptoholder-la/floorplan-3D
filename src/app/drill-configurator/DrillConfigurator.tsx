...
import { useMobile } from '@/hooks/use-mobile'; // Ensure this is correctly imported
          ...
          <div className="space-y-6">
            ...
            <Card className="p-6 bg-white dark:bg-[#182334] border-slate-200 dark:border-white/5 rounded-2xl shadow-sm">
              ...
              <div className="space-y-3">
                ...
                <div>
                  <Label>Type</Label>
                  <Select value={holeType} onValueChange={(v) => {
                    // Ensure v is one of the allowed types
                    if (v === 'through' || v === 'blind' || v === 'countersink') {
                      setHoleType(v);
                    } else {
                      toast.error(`Invalid hole type: ${v}`);
                    }
                  }}>
                    <SelectTrigger>
                      <SelectValue />
              ...
              <TabsContent value="linear" className="space-y-4 mt-4">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="col-span-2">
                    ...
                    <Select value={linearDirection} onValueChange={(v: any) => {
                      if (v === 'horizontal' || v === 'vertical') {
                        setLinearDirection(v);
                      } else {
                        toast.error(`Invalid linear direction: ${v}`);
                      }
                    }}>
                      <SelectTrigger>
                        <SelectValue />
            ...
            <ScrollArea className="h-[400px]">
              <div className="space-y-2">
                {patterns.map((pattern) => (
                  <div
                    key={pattern.id}
                    className={`p-4 rounded-lg border cursor-pointer transition-all ${selectedPatternId === pattern.id
